...
[[nodiscard]] int myFunc() { return 42; }

[[nodiscard("Reason why you should not discard the return value")]] int
myFunc2() {
    return 42;
}
...
int main(int argc, char **argv) {
    ...
    std::byte b{72};
    cout << "using just cout -> std::byte b{72} = "
         << std::to_integer<unsigned int>(b) << "\n";
    // cout << std::format("using std::format -> std::byte b{72} = {}",
    // std::to_integer<unsigned int>(b)) << '\n';
    cout << std::format("using std::format -> std::byte b {{72}} = {}",
                        std::to_integer<unsigned int>(b))
         << '\n';
    ...
    int i5 =
        large_float; // implicit casting with a larger value than int can store.
    ...
    // infinty / NaN page 66
    // NOTE: The integer division 0/0 is un undefined behaviour in C++, so
    // std::isnam returns false.
    int i6 = 0 / 0;
    ...
    cout << "float infinity = " << std::numeric_limits<float>::infinity()
         << endl;
    cout << "double infinity = " << std::numeric_limits<double>::infinity()
         << endl;
    ...
    // NOTE: It makes no sense get remaining of float / double values in a
    // division and it's an error in C++
    ...
    // if PieceType.Queen == 2 // it is an invalid code. You cannot compare
    // enums.
    ...
    // old enum style. It has a problem of names clashing. The example below
    // does not compile. ALWAYS use stronged typed `enum class`.
    /*
     * bool ok { false };
     * enum status { ok, nok }
     * cout << "ok ? " << ok << endl;
    ...
    /* We cannot use strong_ordering and parcial_ordering cannot be used on
    `switch` neither with `using enum` switch(result) { case
    std::strong_ordering::less: cout << "4 is less than 5\n"; case
    std::strong_ordering::equal: cout << "4 is equal 5\n"; case
    std::strong_ordering::greater: cout << "4 is greater than 5\n";
    }
    */

    result = 28 <=> 7;
    if (std::is_lt(result)) {
        cout << "28 is greater than 7\n";
    }
    if (std::is_eq(result)) {
        cout << "28 is equal 7\n";
    }
    ...
    // the compiler will issue an warning due the [[nodiscard]] attribute added
    // to it.
    myFunc();
    myFunc2();
